...
- 11 bundled, rights-reviewed sessions: three music tracks, three Chinese female readings, one Chinese female science reading, three steady soundscapes, and one international female body scan.
- About 35.7 MB of audio assets, all available offline; unreviewed platform and direct-stream entries have been removed from the production playback path.
- A separate 167-item public candidate queue is kept for developer review. Normal app startup does **not** load or expose it; tests/tooling must opt in explicitly.
...
The reviewed catalog is maintained in [`assets/content/audio_catalog.json`](assets/content/audio_catalog.json). A separate review queue lives in [`assets/content/audio_candidates.json`](assets/content/audio_candidates.json) and is generated by [`tools/build_audio_candidates.py`](tools/build_audio_candidates.py). The normal application startup path does not load the candidate queue. Candidates are inspected by developer tooling and may only be promoted after complete listening and rights review.
