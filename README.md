...
- A JSON-maintained audio catalog with source, creator, license, region, checksum, duration, and evidence-role metadata.
- 12 bundled, rights-reviewed sessions: three music tracks, three Chinese female readings, one Chinese female science reading, three steady soundscapes, one international female body scan, and one slow-breathing pacer.
- About 63.8 MB of bundled audio assets, all available offline, plus a separate four-item opt-in knowledge library using direct HTTPS audio.
- A separate in-app testing library exposes 167 automatically screened public candidates. They stay clearly marked as unreviewed and never enter automatic recommendations.
...
The reviewed catalog is maintained in [`assets/content/audio_catalog.json`](assets/content/audio_catalog.json). The opt-in knowledge library lives in [`assets/content/study_drowsy_catalog.json`](assets/content/study_drowsy_catalog.json). A separate listening queue lives in [`assets/content/audio_candidates.json`](assets/content/audio_candidates.json) and is generated by [`tools/build_audio_candidates.py`](tools/build_audio_candidates.py). The app exposes candidates only through the clearly labelled testing library; candidates remain outside automatic recommendations and may only be promoted after complete listening and rights review.

Approved audio is also assigned one evidence role: trial-aligned music, guided relaxation, supporting music, contextual masking, or comfort-only spoken content. The policy is documented in [`docs/audio_science_standard.md`](docs/audio_science_standard.md). These roles guide recommendation behavior; they are not claims that an individual recording is clinically proven.

Run the local gate before promoting or editing audio:

```bash
python tools/audit_sleep_audio.py
```

The audit checks role boundaries, bundled-file SHA256 values, short-loop flags, and candidate-queue assumptions.
...
python tools/audit_sleep_audio.py
python -m unittest tools/test_audit_sleep_audio.py tools/test_build_audio_candidates.py
flutter build apk --debug
```

The automated suite covers recommendations, evidence-role boundaries, candidate isolation, regional catalogs, asset assumptions, local preference persistence, offline-download guards, dream boundaries, 30-day retention, nap-safe health aggregation, health-record minimization, and core widgets.
